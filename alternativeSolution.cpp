...
int main()
{
	...
	while (1) {
		...
		int units = 5;
		vector<int> moves;
		// a function to define the moves

		while(units > 0) {
			for (int i = 0; i < planets.size(); i++) {
				if (units == 0) break;
				auto p = planets[i];
				if (p._canAssign && p._myTolerance > 0) {

					if (p._myUnits > p._otherUnits) {
						continue;
					}
					else if (p._otherUnits > p._myUnits) {
						int difference = p._otherUnits - p._myUnits;
						if (difference >= 5 && units ==5) {
							for(int j=0 ; j < 5 ; j++)
								moves.push_back(j);
							units = 0;
						}
						else if(units - difference > 0){
							for(int j = 0; j < difference; j++)
								moves.push_back(j);
							units = units - difference + 1;
						} 
					}
					else {
						moves.push_back(1);
		...
		// a function to define the spreads, spread on the planet with max units
